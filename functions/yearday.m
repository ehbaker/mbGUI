...
if isequal(theYear, 'demo')
	...
	b = yearday(yearday(a));
	disp(datestr(b))
	year_day_round_trip_error = b-a %#ok<NOPRT,NASGU>
	return
end
...
% elseif length(theDate) > 1   % the original version
% 	for i = 1:length(theDate)
% 		v{i} = theDate(i);
% 	end
% 	theDate = datenum(v{:});
% end
elseif n > 1                   % my version requires the datevector being horizonatl
	theDate = datenum(theDate);
end

d = datevec(theDate);
%d(2:6) = [1 1 0 0 0];   % January 1, midnight.
d(:,2:6) = repmat([1 1 0 0 0],m,1);   % January 1, midnight.
% for i = 1:length(d)
% 	v{i} = d(i);
% end
